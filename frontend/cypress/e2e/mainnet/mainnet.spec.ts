import { dropWebSocket, emitMempoolInfo } from '../../support/websocket';
...
describe('Mainnet', () => {
  ...
  if (baseModule === 'mempool') {
    ...
    describe('search', () => {
      it('allows searching for partial BBQCoin addresses', () => {
        cy.visit('/');
        ...
        cy.get('app-search-results button.dropdown-item.active').click().then(() => {
          ...
          cy.waitForSkeletonGone();
          cy.get('.text-center').should('not.have.text', 'Invalid BBQCoin address');
        });
      });
